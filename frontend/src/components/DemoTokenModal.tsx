...
const DemoTokenModal: React.FC<Props> = ({ onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-black/60 backdrop-blur-sm grid place-items-center z-50 p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 md:p-10 grid gap-6">
        <div className="flex flex-col items-center text-center gap-3">
          <div className="bg-gradient-to-br from-blue-500 to-purple-600 p-4 rounded-2xl shadow-lg">
            <Lock className="w-8 h-8 text-white" />
          </div>
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Acceso a la Demo</h2>
          <p className="text-gray-600 text-sm md:text-base">Introduce tu token de acceso para usar esta demo.</p>
        </div>
